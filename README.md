...
- **关节电机**：宇树A1
    ...
    - 连接方式：一个串口连接一个电机实现控制频率到达1000hz(1000hz会出现bug，最终将频率降为500hz)
...
- **2025.02.07**：今天的调试发现了太多问题了，车子还会莫名其妙抽搐，实在没辙了，优化代码提高代码效率试试看，自启动还是有问题，可能是打滑？还有其他很多问题，今天烦心事太多了，不想调了。
- **2025.02.27**：在将电机频率降到500hz之后，仍旧出现掉帧的情况，最后将串口陀螺仪频率改为500hz，波特率改为420800后，终于解决掉帧的bug，此后控制频率改为500hz完全够用。调试过程中目标腿长和实际腿长差别较大，检查发现在计算支持力F0时，前馈Mg机体质量应该除二，修改后腿长正常。调试过程中还发现关节电机很软，很轻松就能把车按扁，增大腿长pd的p会变硬，但仍旧不够，后续需要继续增大调试，还有腿车轮毂也比较软，腿轻轻踹就能踢开车，效果远远不好，仍需调试。另外今天调试的时候撞碎了屏幕，心疼。
- **2025.03.02**：目前轮腿的基本运动没有太大问题了，在屋内调试打滑较多，但是轮腿关节依旧比较绵软，经过调试和分析猜测是LQR的R矩阵需要修改，下一步修改R和Q矩阵（主要是R矩阵）观察效果。还有yaw轴动不动会出现自己突然旋转半圈的情况，推测是陀螺仪yaw轴出现问题，需要后续打印数据观察，还有yoll轴补偿，跳跃等等需要调试。
- **2025.03.03**：将输入矩阵R矩阵等比例缩小后，对于轮毂电机输出力矩限制更小，导致车子出现抖动，因此需要加大R矩阵。
- **2025.03.05**：经过多次调试已经逐渐找到调试经验，Q矩阵大概调一下权重就可以，重点需要调试R矩阵，并且今天发现不论怎么调都很难有好的效果根本原因就是轮子打滑，最后决定重新发一套轮毂，包胶硬度选择60。
- **2025.03.19**：更新陀螺仪yaw轴过零处理。
- **2025.03.24**：学习ROS2。
- **2025.03.25**：学习ROS2。
- **2025.03.26**：学习ROS2。
- **2025.03.27**：学习ROS2。
- **2025.03.28**：学习ROS2。
- **2025.03.29**：创建腿车工作空间并创建节点，移植陀螺仪ROS2代码。
- **2025.03.31**：修改imu话题消息接口，扩展yaw、roll和pitch消息，并成功发布和订阅。
- **2025.04.01**：创建底盘订阅话题节点和主任务节点，成功订阅imu话题。
- **2025.04.02**：学习linux的SocketCAN并编写can通讯代码，但是板子can接口线材需要购买，需要等到线材到了才能用上位机测试代码是否正确。为ROS2安装串口库serial，等第二天再编写代码测试串口通讯。
